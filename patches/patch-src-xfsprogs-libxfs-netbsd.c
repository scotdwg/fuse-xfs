...
+	}
+
+	if (prop_dictionary_recv_ioctl(fd, DIOCGDISKINFO, &dict) == 0) {
+		int64_t tmpsz;
+		dict = prop_dictionary_get(dict, "geometry");
+		prop_dictionary_get_int64(dict, "sectors-per-unit", &tmpsz);
+		prop_dictionary_get_uint32(dict, "sector-size", bsz);
+		*sz = tmpsz;
+		return;
+	}
+
+	if (ioctl(fd, DIOCGDINFO, &disklabel) == 0) {
+		*sz = disklabel.d_secperunit;
+		*bsz = disklabel.d_secsize;
+		return;
+	}
+
+	fprintf(stderr, _("%s: Can't get sizes for \"%s\"\n"), progname, path);
+	exit(1);
+}
+
+char *
